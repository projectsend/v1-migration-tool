...
**A v1 download is recorded twice, and only one of the two is imported.** `record_new_download()` writes a `tbl_downloads` row, and the very same request then writes an actions_log row — code 7 or 8 through `Download.php`, code 37 through `download.php` for an anonymous one. Both describe one download. `DownloadsPhase` and `ActivityLogPhase` each read their own table and both wrote v2 `file.downloaded` / `public_file.downloaded` rows, so every migrated install reported close to twice the downloads it actually had, in v2's own product — the file details panel, `/downloads`, and the admin dashboard chart all count those actions.

Verified against the fixtures rather than reasoned about: `tbl_downloads` and the download codes in `tbl_actions_log` come to 314/314 on `ps-small` and 99,473/99,473 on `ps-large`, and joining the two on file plus timestamp matches every row. (`ps-messy` reads 1,076/879 for an unrelated reason — `phaseExtras()` in the seeder injects downloads-only rows to drive files past their limit. That gap is the fixture doing its job, and it must not be "corrected".)

So `ActionMap` drops 7, 8 and 37, and `DownloadsPhase` owns downloads outright. The downloads table is the right survivor rather than an arbitrary one: it is the ledger v1's *own* numbers come from — `manage-files.php`'s `download_count`, the limit checks in `functions.php` — so reading it alone reproduces exactly what the customer saw in v1, and it carries the IP and the anonymous flag that the log row does not. They are dropped with a reason in the report rather than deleted from the map, because three download codes missing without explanation reads as an oversight to be filled back in.

Two consequences worth stating plainly. `DownloadsPhase` had been writing `actor_name` and `subject_name` as null, which was survivable only while the log rows were also arriving with them filled in; as the sole source it now snapshots both, read from the accounts and files this migration has already written, which is also what v2 itself records for a download happening now. Without them v2 prints "(deleted account)" beside an account that is sitting right there, and the downloads screen's file and user filters — which match on those columns rather than joining — find nothing. And when v1's `download_logging_ignore_file_author` is on, an author downloading their own file writes the log row but *not* the ledger row, so those downloads no longer come across: that is the same arithmetic v1 did, since the option exists precisely to keep them out of v1's counts, so the number stays right and an audit line is lost. The option is off by default.

**Download limits carry, but their spent counts are not carried — they are recomputed.** v1's three columns (`download_limit_enabled`, `download_limit_type`, `download_limit_count`) become v2's two (`download_limit` nullable, `download_limit_scope`), and the two scope names happen to match exactly. Nothing copies "how many downloads have already been used", because v2 derives that from its activity log, and `DownloadsPhase` has already put v1's download history there — actor and all, so a `per_user` limit arrives correctly spent per person. That this is now the *only* source of that history matters here too: while both phases were importing downloads, every limited file also arrived with roughly twice its allowance spent. The one case to know about is `--history=none`: no downloads imported means every limited file arrives with a full allowance.
...
**It restores the names as well as the count**, and that half was found by running it rather than by reading it. The old `DownloadsPhase` wrote `subject_name` and `actor_name` as null, which was survivable only while the duplicate beside it carried them; deleting that duplicate makes it permanent, and v2 then prints "(deleted account)" next to an account that exists and hides the row from the downloads screen's two filters, which match those columns rather than joining. The names are read back from the accounts and files this migration already wrote — the same source the current `DownloadsPhase` uses, so a repaired install reads the same as one migrated by a current release. A download whose file has been deleted since has nothing to restore from and is not counted as outstanding, or the command would report work it can never finish.
...
**Verified end to end against `ps-small`** before v1.0.4 was tagged, using the released v1.0.3 to do the import so the duplicates were real rather than constructed: 314 v1 downloads became 628 rows, the repair took them back to 314 with every IP intact, 26 anonymous rows matching v1's 26, 288 named actors (the other 26 being those anonymous ones), and 526 unrelated activity rows untouched. Running it again reports nothing to do.
...
| A password v2 cannot verify | **A note, not a blocker.** v1 hashes with bcrypt and the digest carries across, so almost everyone keeps their password. Two shapes need handling. A digest labelled `$2a$` or `$2b$` — the same algorithm under an older PHP's name for it — is **relabelled** to `$2y$`, four bytes changed and salt and digest untouched, because v2's hasher decides what it recognises with `password_get_info()`, which answers "unknown" for both even though `password_verify()` reads them perfectly well. Skipping that relabel was projectsend/projectsend#1706: a migrated installation where every account met a 500 at the login form. A blank password, one left by a pre-bcrypt ProjectSend, or a `$2x$` digest (which asks for the *old, broken* handling of bytes above 127 on purpose, so it cannot be relabelled) genuinely cannot be verified: those rows get a bcrypt hash of 64 random characters instead, one each, which turns the 500 back into "these credentials do not match". Nothing usable is lost, because v1 authenticates with `password_verify()` and could not read those digests either. Named before the run and counted during it; those people use "forgot password" once |
...
| `tests/Unit/LegacyPasswordTest.php` | Which v1 digests v2 can verify — `$2y$` at any cost, `$2a$` and `$2b$` once relabelled, neither a blank password nor another algorithm nor `$2x$` — that relabelling preserves the password itself across ASCII, accented, high-bit and multi-byte inputs, and that the digests it cannot read are each replaced with a distinct hash nobody holds |
